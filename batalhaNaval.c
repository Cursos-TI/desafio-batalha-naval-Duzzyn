...
bool valida_posicao(int linha, int coluna, Orientacao orientacao) {
    switch (orientacao) {
        case HORIZONTAL:
            // Início no tabuleiro e fim (coluna + SHIP_SIZE - 1) antes do limite
            return (linha >= 0 && linha < TAB_SIZE &&
                    coluna >= 0 && coluna + SHIP_SIZE <= TAB_SIZE);
        case VERTICAL:
            // Início no tabuleiro e fim (linha + SHIP_SIZE - 1) antes do limite
            return (linha >= 0 && linha + SHIP_SIZE <= TAB_SIZE &&
                    coluna >= 0 && coluna < TAB_SIZE);
        case DIAGONAL_DESC:
            // Fim (linha + SHIP_SIZE - 1) e (coluna + SHIP_SIZE - 1) antes do limite
            return (linha >= 0 && linha + SHIP_SIZE <= TAB_SIZE &&
                    coluna >= 0 && coluna + SHIP_SIZE <= TAB_SIZE);
        case DIAGONAL_ASC:
            // Linha inicial (linha) deve estar no tabuleiro (linha < TAB_SIZE).
            // Linha final (linha - SHIP_SIZE + 1) deve ser >= 0.
            // Coluna final (coluna + SHIP_SIZE - 1) deve ser < TAB_SIZE.
            return (linha >= 0 && linha < TAB_SIZE &&
                    linha - SHIP_SIZE + 1 >= 0 && 
                    coluna >= 0 && coluna + SHIP_SIZE <= TAB_SIZE);
        default:
...
// Função para verificar sobreposição do navio no tabuleiro (restante do código omitido por ser idêntico e correto)
...
// Função para posicionar o navio no tabuleiro (restante do código omitido por ser idêntico e correto)
